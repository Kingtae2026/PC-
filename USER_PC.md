...
카운터 PC의 TCP 서버(Port 9000/9001)에 연결하여 로그인/로그아웃, 남은시간 확인, 음식 주문, 관리자 채팅 문의 기능을 제공합니다.

<br>

## 화면 흐름

![화면 흐름](https://github.com/user-attachments/assets/21991865-aa2c-4f84-9cfd-0df096a02147)
...
`UserPcLogin.cs` — 카운터 PC 서버(Port 9000)로 로그인을 요청합니다.
...
`Menu.cs` — 카운터 PC 서버(Port 9001)로 주문을 전송합니다.
...
`ChatForm.cs` — 카운터PC(아르바이트)에게 실시간 메시지를 보낼 수 있습니다.
...
- 카운터PC(아르바이트) 측에서는 해당 좌석 버튼이 노란색으로 강조되어 알림
...
| `CHAT_POLL\|id` | →서버 | 카운터PC(아르바이트) 답장 수신 확인 |
...
| `ChatForm.cs` | 카운터PC(아르바이트) 1:1 채팅 UI |
